  ...
  export class Simulation {
    ...
    // simulation.ts – dentro da classe Simulation
...
private _canEnter(car: Car) {
  ...
  // Verifica se há algum carro no cruzamento com direção que cruza a deste carro
  const crossingOccupied = this.cars.some(c =>
    c !== car && c.state !== "done" && c.isInIntersection() && this._directionsCross(car.direction, c.direction)
  );

  return this.lights[car.direction].isGreen() && !crossingOccupied;
}
